...
class RetroEffectImpl extends Effect {
  private _colorNum: number = 4.0;
  private _pixelSize: number = 2.0;
...
function RetroEffect({ colorNum, pixelSize }: { colorNum: number; pixelSize: number }) {
  ...
  return effectRef.current ? <primitive object={effectRef.current} /> : null;
}
  ...
  return (
    <>
      <mesh ref={mesh} scale={[viewport.width, viewport.height, 1]}>
        ...
        {shaderMaterialRef.current && <primitive object={shaderMaterialRef.current} attach="material" />}
      </mesh>
      ...
      >
        <planeGeometry args={[1, 1]} />
        <primitive object={new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 })} attach="material" />
      </mesh>
    </>
  );
}
